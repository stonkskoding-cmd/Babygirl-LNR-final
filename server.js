...
if (!MONGODB_URI) {
  console.error('❌ ОШИБКА: MONGODB_URI не задан!');
  process.exit(1);
}
...
// --- CHAT CLIENT (ИСПРАВЛЕНО: ПРИВЕТСТВИЕ ПЕРВЫМ) ---
  ...
  try {
    ...
    if (!chat) {
      ...
      console.log('💬 New chat created with greeting for:', req.user.username);
       ...
       await chat.save();
       console.log('💬 Greeting added to empty chat for:', req.user.username);
    }
...
// --- ADMIN CHATS (ИСПРАВЛЕНО: ПОЛНАЯ ИСТОРИЯ) ---
...
// КРИТИЧЕСКИ ВАЖНО: Возвращаем ПОЛНЫЙ чат со всеми сообщениями
...
app.put('/api/admin/chat/:userId/complete', authenticate, isAdmin, async (req, res) => {
  try {
    ...
    console.log('✅ Chat completed (bot reset, history saved)');
...
app.put('/api/admin/chat/:userId/clear', authenticate, isAdmin, async (req, res) => {
  try {
    ...
    console.log('🗑️ Chat cleared (history deleted)');
    res.json({ success: true });
  } catch (e) {
    res.status(500).json({ message: e.message });
  }
